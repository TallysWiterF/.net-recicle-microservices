...
namespace MensageriaRabbitMq.Setup
{
    public class Rabbit : IRabbit
    {
        ...
        private void CriarSetupDeadLetter(IModel canal)
        {
            ...
            {
                {"x-dead-letter-exchange", Filas.DEAD_LETTER_COLETA.ToString() }
            };
            canal.ExchangeDeclare(Filas.DEAD_LETTER_COLETA.ToString(), ExchangeType.Fanout);
            CreateQueue(Filas.DEAD_LETTER_COLETA.ToString(), argumentos, canal).Wait();
            canal.QueueBind(Filas.DEAD_LETTER_COLETA.ToString(), Filas.DEAD_LETTER_COLETA.ToString(), string.Empty);
        }
        #endregion
    }
}
