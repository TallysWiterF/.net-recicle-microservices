...
namespace MensageriaRabbitMq.Setup
{
    public class Rabbit : IRabbit
    {
        ...
        private void CriarSetupDeadLetter(IModel canal)
        {
            ...
            {
                {"x-dead-letter-exchange", Filas.DEAD_LETTER_PERFIS.ToString() }
            };
            canal.ExchangeDeclare(Filas.DEAD_LETTER_PERFIS.ToString(), ExchangeType.Fanout);
            CreateQueue(Filas.DEAD_LETTER_PERFIS.ToString(), argumentos, canal).Wait();
            canal.QueueBind(Filas.DEAD_LETTER_PERFIS.ToString(), Filas.DEAD_LETTER_PERFIS.ToString(), string.Empty);
        }
        #endregion
    }
}
