...
namespace MensageriaRabbitMq.Setup
{
    public class Rabbit : IRabbit
    {
        ...
        private void CriarSetupDeadLetter(IModel canal)
        {
            ...
            {
                {"x-dead-letter-exchange", Filas.DEAD_LETTER_ACESSO.ToString() }
            };
            canal.ExchangeDeclare(Filas.DEAD_LETTER_ACESSO.ToString(), ExchangeType.Fanout);
            CreateQueue(Filas.DEAD_LETTER_ACESSO.ToString(), argumentos, canal).Wait();
            canal.QueueBind(Filas.DEAD_LETTER_ACESSO.ToString(), Filas.DEAD_LETTER_ACESSO.ToString(), string.Empty);
        }
        #endregion
    }
}
